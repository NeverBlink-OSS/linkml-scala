...
class JsonSchemaIntegrationSpec extends AnyWordSpec, Matchers, ModelCatalogueSpec {
  ...
  "Json Schema generator" should {
    for includeNull <- Seq(false, true) do
      s"includeNull=$includeNull" when {
        "Optional scalar nulls" in {
          val options =
        ...
        "Simple root dictionary requirements" in {
        ...
        "Inherited dictionary requirements" in {
          val schemaYaml =
            """id: https://example.org/linkml/inherited-required
    ...
    "Respect required content in root dictionary entries" in {
      val options = JsonSchemaGenerator.Options(
        treeRootInlineType = Some("compact_dict"),
        includeNull = true,
      )
