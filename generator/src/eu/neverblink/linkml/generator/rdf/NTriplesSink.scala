 ...
 * emitted verbatim. This ASCII guarantee lets the JVM sink take a fast byte path (see
 * `BufferedByteSink` in the JVM source set).
 */
trait NTriplesSink {
...
final class StringNTriplesSink extends NTriplesSink {
  private val sb = new scala.collection.mutable.StringBuilder
