 ...
 *
 * In a program Scala Native linked itself that runs near the base of the stack and the
 ...
 * It is just never called for a thread the runtime did not start. Calling it from the thread that
 * will own the library, right after loading, is the whole fix.
 *
 * See scala-native#4334, which reports these symptoms from Swift without reaching the cause.
 */
...
/* Big enough to look like a normal thread stack. Only sizes the stack-overflow guard. */
