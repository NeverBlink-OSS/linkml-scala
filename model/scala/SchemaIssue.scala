...
/** A single issue found during the validation of a LinkML schema.
  ...
  * @note
  *   This class is intended to be subclassed for specific types of issues. The issues should declare additional slots that are specific to the type of issue (e.g., name of class).  The `message` and `details` slots should be specified in the subclass using `equals_expression`  to provide a human-readable message describing the issue.
  * @note
  *   Do not subclass this class directly. Instead, subclass one of the following classes: SchemaWarning, SchemaError, SchemaFatal.
  */
